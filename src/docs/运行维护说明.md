# 运行维护说明

本文档整理工程环境、编译、systemd 自启动、日志查看、网页监控和临时文件共享。

## 1. 环境

```bash
cd /root/GZ_DiNiu
source /opt/ros/humble/setup.bash
source install/setup.bash
```

## 2. 编译

全量编译：

```bash
colcon build
```

常用单包编译：

```bash
colcon build --packages-select motor_control_py
colcon build --packages-select ackermann_odom
colcon build --packages-select steer_closed_loop
```

编译后重新加载环境：

```bash
source install/setup.bash
```

## 3. systemd 自启动

目标：

- 开机自动启动 `pigpiod`
- 开机自动启动 `ros2 launch motor_control_py cmd_vel_full.launch.py`

已配置服务：

- `/etc/systemd/system/pigpiod.service`
- `/etc/systemd/system/ros2-motor-control.service`

`ros2-motor-control.service` 的启动命令：

```bash
source /opt/ros/humble/setup.bash
source /root/GZ_DiNiu/install/setup.bash
ros2 launch motor_control_py cmd_vel_full.launch.py start_pigpiod:=false
```

`start_pigpiod:=false` 用于避免 launch 文件重复拉起 `pigpiod`。

启用与启动：

```bash
sudo systemctl daemon-reload
sudo systemctl enable --now pigpiod.service
sudo systemctl enable --now ros2-motor-control.service
```

## 4. 服务管理

查看状态：

```bash
systemctl status pigpiod.service --no-pager -l
systemctl status ros2-motor-control.service --no-pager -l
```

重启：

```bash
sudo systemctl restart pigpiod.service
sudo systemctl restart ros2-motor-control.service
```

停止：

```bash
sudo systemctl stop ros2-motor-control.service
sudo systemctl stop pigpiod.service
```

开机自检：

```bash
systemctl is-active pigpiod.service
systemctl is-active ros2-motor-control.service
```

期望输出均为 `active`。

## 5. 日志

跟随查看：

```bash
journalctl -u pigpiod.service -f
journalctl -u ros2-motor-control.service -f
```

查看最近日志：

```bash
journalctl -u ros2-motor-control.service --no-pager -n 80
```

`motor_control_node` 的 `/cmd_vel` 日志默认节流：

- `cmd_log_period_s=1.0`：同类命令最多每 1 秒打印一次
- `cmd_log_delta_threshold=0.02`：目标速度、角速度或转角变化明显时立即打印
- `drive_log_period_s=1.0`：驱动闭环状态最多每 1 秒打印一次
- `drive_log_delta_threshold=0.03`：驱动反馈、误差或 PWM 输出变化明显时立即打印
- `feedback_timeout_s=0.5`：速度反馈超过 0.5 秒未更新时强制驱动 PWM 为 0
- `zero_target_active_brake=false`：停车命令直接关闭驱动 PWM，不使用反向 PWM 主动刹车

停车后轻微后退排查：

- 先确认配置中 `zero_target_active_brake=false`
- 发送停车命令后查看驱动日志，正常应为 `out%=0.00 duty=0`
- 若日志出现负向 `out%`，表示仍在主动反向刹车，需要检查参数是否已重新编译并重启服务
- 若日志为 `out%=0.00 duty=0` 但车辆仍后退，优先检查电机驱动器制动模式、机械惯性和速度反馈方向

如需完整抓取每条 `/cmd_vel`：

```bash
ros2 run motor_control_py motor_control_node --ros-args -p cmd_log_period_s:=0.0
```

## 6. 网页监控

启动速度网页监控：

```bash
source /opt/ros/humble/setup.bash
source /root/GZ_DiNiu/install/setup.bash
pkill -f speed_web_node || true
ros2 run motor_control_py speed_web_node
```

浏览器打开：

```text
http://<设备IP>:8080
```

可选参数：

```bash
ros2 run motor_control_py speed_web_node --ros-args \
  -p web_host:=0.0.0.0 \
  -p web_port:=8080 \
  -p cmd_vel_axis:=x \
  -p cmd_vel_scale:=1.0
```

## 7. IO 控制节点

功能包：

- `io_control_py`

默认映射：

- 按钮 1 -> BCM27
- 按钮 3 -> BCM17
- `active_low:=true`，激活时输出低电平

直接运行：

```bash
ros2 run io_control_py io_control_node
```

一键启动中默认已启用；如需关闭：

```bash
ros2 launch motor_control_py cmd_vel_full.launch.py enable_io_control:=false
```

状态话题：

```bash
ros2 topic echo /gpio/bcm27_active
ros2 topic echo /gpio/bcm17_active
```

命令话题：

```bash
ros2 topic pub -1 /gpio/bcm27_set std_msgs/msg/Bool "{data: true}"
ros2 topic pub -1 /gpio/bcm27_set std_msgs/msg/Bool "{data: false}"
ros2 topic pub -1 /gpio/bcm17_set std_msgs/msg/Bool "{data: true}"
ros2 topic pub -1 /gpio/bcm17_set std_msgs/msg/Bool "{data: false}"
```

## 8. 调试文件共享

如需临时配置 Samba，建议只共享工程目录或专用调试目录，不建议共享整个 `/root`。

示例共享目录：

```text
/root/GZ_DiNiu
```
